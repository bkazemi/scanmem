...
#define FLAG_MACRO(bytes, string) (val->flags.u##bytes##b &&                   \
    ...
    np = snprintf(buf, sizeof(buf), "[%s%s%s%s%s%s%s]",
         ...
         ( val->flags.ineq_reverse &&
          !val->flags.ineq_forwards) ? "(reversed inequality) " : "");
    ...
    /*
     * find the right format, considering different integer size
     * implementations
     */
    if (max_bytes == sizeof(long long))
        np = snprintf(str, n, print_as_unsigned ? "%llu, %s" : "%lld, %s",
                      print_as_unsigned ? get_ulonglong(val)
                                        : get_slonglong(val), buf);
    else if (max_bytes == sizeof(long))
        np = snprintf(str, n, print_as_unsigned ? "%lu, %s" : "%ld, %s",
                      print_as_unsigned ? get_ulong(val)
                                        : get_slong(val), buf);
    else if (max_bytes == sizeof(int))
        np = snprintf(str, n, print_as_unsigned ? "%u, %s" : "%d, %s",
                      print_as_unsigned ? get_uint(val)
                                        : get_sint(val), buf);
    else if (max_bytes == sizeof(short))
        np = snprintf(str, n, print_as_unsigned ? "%hu, %s" : "%hd, %s",
                      print_as_unsigned ? get_ushort(val)
                                        : get_sshort(val), buf);
    else if (max_bytes == sizeof(char))
        np = snprintf(str, n, print_as_unsigned ? "%hhu, %s" : "%hhd, %s",
                      print_as_unsigned ? get_uchar(val)
                                        : get_schar(val), buf);
    else if (val->flags.f64b)
        np = snprintf(str, n, "%lf, %s", get_f64b(val), buf);
    else if (val->flags.f32b)
        np = snprintf(str, n, "%f, %s", get_f32b(val), buf);
    else
        goto err;
...
bool parse_uservalue_number(const char *nptr, uservalue_t * val)
{
    ...
    if (parse_uservalue_int(nptr, val)) {
        ...
        if (num >= (double)(0) && num < (double)(1LL<< 8)) {
            val->flags.u8b = 1;
            set_u8b(val, (uint8_t)num);
        }
        if (num >= (double)-(1LL<< 7) && num < (double)(1LL<< 7)) {
            val->flags.s8b = 1;
            set_s8b(val, (int8_t)num);
        }
        if (num >= (double)(0) && num < (double)(1LL<<16)) {
            val->flags.u16b = 1;
            set_u16b(val, (uint16_t)num);
        }
        if (num >= (double)-(1LL<<15) && num < (double)(1LL<<15)) {
            val->flags.s16b = 1;
            set_s16b(val, (int16_t)num); }
        if (num >= (double)(0) && num < (double)(1LL<<32)) {
            val->flags.u32b = 1;
            set_u32b(val, (uint32_t)num);
        }
        if (num >= (double)-(1LL<<31) && num < (double)(1LL<<31)) {
            val->flags.s32b = 1;
            set_s32b(val, (int32_t)num);
        }
        if ((double)(true) && (double)(true)) {
            val->flags.u64b = 1;
            set_u64b(val, (uint64_t)num);
        }
        if ((double)(true) && (double)(true)) {
            val->flags.s64b = 1;
            set_s64b(val, (int64_t)num);
        }
...
bool parse_uservalue_float(const char *nptr, uservalue_t * val)
{
    ...
    /*
     * I'm not sure how to distinguish float & double,
     * but I guess it's not necessary here
     */
    val->flags.f32b = val->flags.f64b = 1;
...
#define DEFINE_GET_BY_SYSTEM_DEPENDENT_TYPE_FUNCTION(type, typename,    \
                                                     signedness_letter) \
type get_##signedness_letter##typename (const value_t const* val)       \
{                                                                       \
    switch (sizeof(type)) {                                             \
	case  1: return (type)get_##signedness_letter##8b(val);             \
	case  2: return (type)get_##signedness_letter##16b(val);            \
	case  4: return (type)get_##signedness_letter##32b(val);            \
	case  8: return (type)get_##signedness_letter##64b(val);            \
