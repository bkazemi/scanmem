...
int main(int argc, char **argv)
{
    ...
    if (vars->target == 0) {
        show_user("Enter the pid of the process to search using the \"pid\" "
                   "command.\n");
        show_user("Enter \"help\" for other commands.\n");
    } else {
        show_user("Please enter current value, or \"help\" for other "
                  "commands.\n");
    }
    ...
    while (!vars->exit) {
        ...
        /*
         * reads in a commandline from the user and returns a pointer 
         * to it in *line
         */
        ...
        /*
         * sm_execcommand() returning failure is not fatal, just the
         * command could not complete.
         */
        if (sm_execcommand(vars, line) == false) {
            if (vars->target == 0) {
                show_user("Enter the pid of the process to search using "
                          "the \"pid\" command.\n");
                show_user("Enter \"help\" for other commands.\n");
            } else {
                show_user("Please enter current value or \"help\" for other "
