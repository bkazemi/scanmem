...
/*
 * match old_value against new_value or user_value
 * (or both, depending on the matching type, store the result into save)
 *
 * NOTE: saveflag must be set to 0, since only useful bits are set,
 *       but extra bits are not cleared!
 *
 * address is pointing to new_value in TARGET PROCESS MEMORY SPACE,
 * used when searching for a byte array
 *
