...
allocate_array(matches_and_old_values_array *arr, unsigned long max_bytes)
{
    ...
    if (!(arr = realloc(arr, bytes_to_allocate)))
        return NULL;

    arr->bytes_allocated = bytes_to_allocate;
    arr->max_needed_bytes = max_bytes;

    return arr;
}

matches_and_old_values_array *
null_terminate (matches_and_old_values_array *arr,
                matches_and_old_values_swath *swath)
{
    ...
    if (bytes_needed < arr->bytes_allocated) {
        ...
        if (!(arr = realloc(arr, bytes_needed)))
            return NULL;

        arr->bytes_allocated = bytes_needed;
    }

    return arr;
}

void data_to_printable_string(char *buf, int buf_len,
                              matches_and_old_values_swath *swath,
                              long idx, int str_len)
{
    long swath_len = swath->number_of_bytes - idx;
    /* TODO: what if length is too large ? */
    long max_len   = (swath_len >= str_len) ? str_len
                                            : swath_len;
    int i;
    ...
    for (i = 0; i < max_len; ++i) {
        ...
        byte  = ((matches_and_old_values_swath *)swath)->data[idx + i].old_val;
...
void data_to_bytearray_text (char *buf, int buf_len,
                             matches_and_old_values_swath *swath,
                             long idx, int bytearr_len)
{
    int  i;
    int  bytes_used = 0;
    long swath_len  = swath->number_of_bytes - idx;
    ...
    /* TODO: what if length is too large ? */
    long max_len = (swath_len >= bytearr_len) ? bytearr_len
                                              : swath_len;
    ...
    for (i = 0; i < max_len; ++i) {
        ...
        snprintf(buf + bytes_used, buf_len - bytes_used,
                 (i < max_len-1) ? "%02x " : "%02x", byte);
...
nth_match (matches_and_old_values_array *matches, unsigned n)
{
    ...
    while (reading_swath_idx->first_byte_in_child) {
        ...
        if (flags_to_max_width_in_bytes(
                reading_swath_idx->data[reading_iter].match_info) > 0) {
            if (i == n)
                return (match_location){reading_swath_idx, reading_iter};
        ...
        if (reading_iter >= reading_swath_idx->number_of_bytes) {
            reading_swath_idx =
                local_addr_beyond_last_element(
                    (matches_and_old_values_swath *)reading_swath_idx);
                  ...
                  long *num_matches, region_t *which, bool invert)
{
    ...
    *num_matches = 0;
    ...
    while (reading_swath.first_byte_in_child) {
        ...
        bool in_region = (addr >= which->start &&
                          addr <  which->start + which->size);

        if ((in_region && invert) || (!in_region && !invert)) {
            match_flags flags;

            flags = reading_swath_idx->data[reading_iter].match_info;
             ...
             */
            writing_swath_idx = add_element((matches_and_old_values_array **)
                                      (&matches),
                                      (matches_and_old_values_swath *)
                                       writing_swath_idx, addr,
                                      &reading_swath_idx->data[reading_iter]);
