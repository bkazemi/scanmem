 ...
 * will not print it. However, a NULL longdoc will print
 * "missing documentation" if specific help is requested, if you really
 * dont want it to print anything, use "".
 *
 * Each handler is passed an argv and argc, argv[0] is the command called,
 * along with a pointer to the global settings structure, which contains
 * various settings and lists that handlers are allowed to change. One
 * handler can handle multiple commands, but you need register each command
 * with its own documentation.
 */

#define SET_SHRTDOC "change known matches to specified value"
#define SET_LONGDOC                                                            \
"usage: set [match-id[,match-id,...]=]n[/d] [...]\n"                           \
...
"else is assumed to be decimal. All known matches, along with their\n"         \
"match-ids can be displayed with the `list` command. Multiple match-ids can\n" \
"be specified, separated with commas and terminated with a '=' sign. To set\n" \
"a value continually, for example to prevent a counter from decreasing,\n"     \
"suffix the command with '/', followed by the number of seconds to wait\n"     \
"between sets. Interrupt scanmem with ^C to stop the setting.\n\n"             \
...
bool handler__set(globals_t *, char **, unsigned);
...
"are displayed. The offset is used from the code load addr or region start.\n"\
"This helps bypassing address space layout randomization (ASLR).\n"

bool handler__list(globals_t *, char **, unsigned);

#define DELETE_SHRTDOC "delete a known match by match-id"
#define DELETE_LONGDOC                                                    \
"usage: delete match-id\n"                                                \
"Remove the match `match-id` from the match list. The `match-id` can be\n"\
"found using the `list` command. To delete all matches, see the `reset`\n"\
"command.\n"                                                              \
...
bool handler__delete(globals_t *, char **, unsigned);

#define RESET_SHRTDOC "forget all matches, and reinitialise regions"
#define RESET_LONGDOC                                                        \
"usage: reset\n"                                                             \
"Forget all matches and regions, and reread regions from the relevant maps\n"\
"file. Useful if you have made an error, or want to find a new variable.\n"  \

bool handler__reset(globals_t *, char **, unsigned);

#define PID_SHRTDOC "print current pid, or attach to a new process"
#define PID_LONGDOC                                                            \
"usage: pid [pid]\n"                                                           \
"If `pid` is specified, reset current session and then attach to new process\n"\
"`pid`. If `pid` is not specified, print information about current process.\n" \

bool handler__pid(globals_t *, char **, unsigned);

#define SNAPSHOT_SHRTDOC "take a snapshot of the current process state"
#define SNAPSHOT_LONGDOC                                                      \
"usage: snapshot\n"                                                           \
"Take a snapshot of the entire process in its current state. This is useful\n"\
"if you don't know the exact value of the variable you are searching for,\n"  \
"but can describe it in terms of higher, lower or equal (see commands `>`,\n" \
"`<` and `=`).\n\n"                                                           \
"NOTE: This can use a lot of memory with large processes."

bool handler__snapshot(globals_t *, char **, unsigned);

#define DREGION_SHRTDOC "delete a known region by region-id"
#define DREGION_LONGDOC \
"usage: dregion [!]region-id[,region-id[,...]]\n" \
"Remove the region `region-id` from the regions list, along with any matches\n"\
"affected from the match list. The `region-id` can be found using the\n"       \
"`lregions` command. A leading `!` indicates the list should be inverted.\n"

bool handler__dregion(globals_t *, char **, unsigned);

#define LREGIONS_SHRTDOC "list all known regions"
#define LREGIONS_LONGDOC                                                      \
"usage: lregions\n"                                                           \
...
bool handler__lregions(globals_t *, char **, unsigned);

#define GREATERTHAN_SHRTDOC "match values that have increased " \
                            "or greater than some number"

#define LESSTHAN_SHRTDOC    "match values that have decreased " \
                            "or less than some number"

#define NOTCHANGED_SHRTDOC  "match values that have not changed " \
                            "or equal to some number"

#define CHANGED_SHRTDOC     "match values that have changed " \
                            "or different from some number"

#define INCREASED_SHRTDOC   "match values that have increased at all " \
                            "or by some number"

#define DECREASED_SHRTDOC   "match values that have decreased at all " \
                            "or by some number"

#define GREATERTHAN_LONGDOC                                                   \
"usage: > [n]\n"                                                              \
...
bool handler__decinc(globals_t *, char **, unsigned);

#define VERSION_SHRTDOC "print current version"
#define VERSION_LONGDOC                         \
"usage: version\n"                              \
"Display the current version of scanmem in use."

bool handler__version(globals_t *, char **, unsigned);
...
bool handler__exit(globals_t *, char **, unsigned);

#define HELP_SHRTDOC "access online documentation, use `help command` " \
                     "for specific help"
#define HELP_LONGDOC                                                           \
"usage: help [command]\n"                                                      \
"If `command` is specified, print detailed info about command `command`\n"     \
"including options and usage summary. If `command` is not specified, print a\n"\
"one line description of each command supported."

bool handler__help(globals_t *, char **, unsigned);

#define DEFAULT_SHRTDOC NULL
#define DEFAULT_LONGDOC                                                        \
"When searching for a number, use any notation in standard C language\n"       \
"(leading 0x for hexadecimal, leading 0 for octal, everything else is\n"       \
"assumed to be decimal).\n"                                                    \
"Float numbers are also okay, but will be rounded if scanning integers.\n"     \
...
"By entering the value of the variable as it changes multiple times, scanmem\n"\
"can eliminate matches, eventually identifying where the variable is located." \
"\nOnce the variable is found, use `set` to change its value.\n"

bool handler__default(globals_t *, char **, unsigned);

#define STRING_SHRTDOC "match a given string"
#define STRING_LONGDOC                                                   \
"usage \" <text>\n"                                                      \
...
bool handler__string(globals_t *, char **, unsigned);

#define UPDATE_SHRTDOC "update match values without culling list"
#define UPDATE_LONGDOC                                                         \
"usage: update\n"                                                              \
...
bool handler__update(globals_t *, char **, unsigned);

bool handler__eof(globals_t *, char **, unsigned);

#define SHELL_SHRTDOC "execute a shell command without leaving scanmem"
#define SHELL_LONGDOC                                                        \
...
bool handler__watch(globals_t *, char **, unsigned);
...
bool handler__show(globals_t *, char **, unsigned);

#define DUMP_SHRTDOC "dump a memory region to screen or a file" 
#define DUMP_LONGDOC                                                        \
"usage: dump <address> <length> [<filename>]\n"                             \
"\n"                                                                        \
"If <filename> is given, save the region of memory to the file otherwise \n"\
"display it in a human-readable format.\n"
    
bool handler__dump(globals_t *, char **, unsigned);

#define WRITE_SHRTDOC "change the value of a specific memory location"
#define WRITE_LONGDOC                             \
"usage: write <value_type> <address> <value>\n"   \
"\n"                                              \
...
"\n"                                                                       \
"detect_reverse_change\twhether to (also) search for values that changes\n"\
"\toppositely as given order\n"                                            \
"\t\t\tDefault:0\n"                                                        \
"\tIf you want to use this feature, you can only search for INCREASED or\n"\
"\tDECREASED after initial search\n"                                       \
"\n"                                                                       \
"\tpossible values:\n"                                                     \
"\t0:\tdisabled\n"                                                         \
"\t1:\tenabled\n"                                                          \
...
bool handler__option(globals_t *, char **, unsigned);
